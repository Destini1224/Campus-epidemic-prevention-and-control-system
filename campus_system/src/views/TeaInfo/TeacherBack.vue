    ...
    <div class="container">
      <div class="brand">
        <el-breadcrumb separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item>返校审批</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      <div>
        <div class="aside">
        ...
        <el-main class="main">
          ...
          <!-- 新增弹窗区域 -->
          <el-dialog title="返校申请审批" :visible.sync="dialogFormVisible" width="30%" top="6vh" center>
            <el-form :model="addForm" :rules="rules" ref="addForm" label-width="80px" size="medium">
              ...
              <el-form-item label="计划出发时间" prop="plan_go">
                <el-date-picker type="datetime" format="yyyy 年 MM 月 dd 日 HH时mm分ss秒" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择计划出发时间" v-model="addForm.plan_go" style="width: 100%;" disabled></el-date-picker>
              </el-form-item>
              ...
              <el-form-item label="出发地点" prop="becity">
                <el-input type="text" auto-complete="false" v-model="addForm.becity" placeholder="请填写出发地点" disabled>
                </el-input>
              </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        plan_go: [{ required: true, message: '请输入计划出发时间', trigger: 'blur' }],
        plan_reach: [{ required: true, message: '请输入计划到达时间', trigger: 'blur' }],
        becity: [{ required: true, message: '请填写出发地点', trigger: 'blur' }],
  ...
  methods: {
    ...
    // 获得某条信息
    getInfo() {
      request.get('/back/' + this.id).then(res => {
        this.$nextTick(() => {
          this.addForm = { ...res[0] }
        })
      })
