<template>
  <div class="news">
    <el-breadcrumb class="brand" separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>管理员列表管理</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 搜索区域 -->
    <div class="student-search" style="padding:0 0 10px 0;">
      <el-input style="width:250px;" placeholder="请输入用户名" suffix-icon="el-icon-search" v-model="username" clearable @change="load">
      </el-input>
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          request.post('/admin', this.addForm).then(res => {
            if (res) {
