<template>
  <div class="student">
    <el-breadcrumb class="brand" separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>学生列表管理</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 搜索区域 -->
    <div class="student-search" style="padding:0 0 10px 0;">
      ...
      <el-input style="width:250px;margin-left:5px;" placeholder="请输入班级" suffix-icon="el-icon-edit" v-model="stuClass" clearable @change="load">
      </el-input>
    ...
    <el-dialog title="用户信息" :visible.sync="dialogFormVisible" width="30%" top="6vh" center>
      <el-form :model="addForm" :rules="rules" ref="addForm" label-width="70px" size="medium">
        ...
        <el-form-item label="班级" prop="stuClass">
          <el-input type=" text" auto-complete="false" v-model="addForm.stuClass" placeholder="请输入班级">
          </el-input>
        </el-form-item>
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          request.post('/student', this.addForm).then(res => {
            if (res) {
    ...
    // 获得某条信息
    getInfo() {
      request.get('/student/' + this.id).then(res => {
        this.$nextTick(() => {
          this.addForm = { ...res[0] }
